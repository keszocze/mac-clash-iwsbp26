...
benchmarkName n Config{useModuleFullAdder, useState, useVector, useRotation, useOneHot} =
  intercalate "_" ["benchmark", "MAC", show n, fa, approach, storage, accessing, counting]
  where
    fa = if useModuleFullAdder then "ModuleFA" else "InlineFA"
