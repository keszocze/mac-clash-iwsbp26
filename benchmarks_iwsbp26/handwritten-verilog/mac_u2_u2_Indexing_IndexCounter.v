 ...
 * verilog_BITWIDTH_Indexing_IndexCounter.v.
 *
...
module mac_u2_u2_Indexing_IndexCounter #(parameter WIDTH = 2)
    (
    input clk,
    input rst,
    input ena,
    ...
    //wire [2**$clog2(WIDTH)-1:0] xw, yw;
    
    integer i;
    
    assign xi = (x_cnt < WIDTH) ? x[x_cnt] : 1'b0;
    assign yi = (y_cnt < WIDTH) ? y[y_cnt] : 1'b0;
    assign xy = (cur_state == CARRY) ? 1'b0 : xi & yi;
    assign pi = p[p_cnt];
    assign c_in = carry;
    ...
    begin
        if (rst || cur_state == IDLE) begin
            p_cnt <= 0;
            p_next_row <= 1;
        end else if (cur_state == ROW || (cur_state == ACCUM && p_cnt != 2*WIDTH-1)) begin
            p_cnt <= p_cnt + 1;
        end else if (cur_state == CARRY) begin
